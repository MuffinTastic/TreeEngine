	...
	*/
	...
	internal static string Generate( in ProjectManifest manifest )
	{
		...
		// Basic configuration.
		{
			...
			basics.CreateElementWithInnerText( "RootNamespace", project.DefaultNamespace ?? "Tree" );
		}

		// Implicit usings.
		{
			var usings = rootElement.CreateElement( "ItemGroup" );
			if ( project.UseTreeGlobal ?? true )
				usings.CreateElementWithAttributes( "Using", "Include", "Tree.Engine.Global", "Static", "true" );
		...
		// Tree references.
		{
			...
			references.CreateElementWithAttributes( "Reference", "Include", baseReferenceDir + "Tree.Engine.dll" );
		}
